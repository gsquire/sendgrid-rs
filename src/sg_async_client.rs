...
use errors::{ SendgridResult, SendgridError };
use mail::Mail;
            ...
            .and_then(|res| {
                res.into_body()
                    ...
                    .and_then(move |body| {
                        ...
                        // copy body to bytes
                        let _ = std::io::copy(&mut body, &mut bytes);
                        std::str::from_utf8(&bytes)
                            .map(String::from)
                            .map_err(|e| SendgridError::from(e.to_string()))
                    })
            })
    }
}
